...
- **Fixes du 2026-07-18 (non committés, delta phase harness)** :
  ...
  - **Incohérence matrice modèle corrigée** : astro/nextjs/react étaient en `sol` (oubli de propagation du reversal sol→terra) → passés `terra`. Distribution périmée depuis le 2026-09-02 — voir `docs/workflow/agents.md` (Model Policy) pour la matrice courante (20 sol / 12 terra / 5 luna ; xhigh conservé pour `design-expert`) ; fixture `agent-toml.test.ts` mise à jour. (`changelog-watcher`+`commit` restent sol = choix délibéré, à confirmer owner.)
