...
- **Fixes du 2026-07-18 (non committés, delta phase harness)** :
  ...
  - **Incohérence matrice modèle corrigée** : astro/nextjs/react étaient en `sol` (oubli de propagation du reversal sol→terra) → passés `terra`. Distribution actuelle **7 sol / 23 terra / 7 luna** (37 agents) ; fixture `agent-toml.test.ts` mise à jour. (`changelog-watcher`+`commit` restent sol = choix délibéré, à confirmer owner.)
