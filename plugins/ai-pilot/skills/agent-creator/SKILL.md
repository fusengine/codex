...
|---------|---------|-------------------|
| `gpt-5.6-terra` | The 12 framework/stack experts (astro, go, laravel, nextjs, php, react, rust, shadcn-ui, swift, tailwindcss, tanstack-start, typescript) | `medium` |
| `gpt-5.6-sol` | The 16 analysis/research/orchestration/release/validation agents (explore-codebase, research-expert, brainstorming, solid-orchestrator, commit, changelog-watcher, lessons-compactor, seo-expert, seo-content, seo-geo, seo-local, seo-cluster, seo-technical, seo-schema, websearch, sniper) | `medium` |
| `gpt-5.6-sol` | One-shot-correctness gates (`challenger`, `security-expert`, `prompt-engineer`) | `high` |
| `gpt-5.6-sol` | Highest visual/product-direction judgment (`design-expert`) | `xhigh` |
...
Terra/`medium` (Terra's default effort) is the executor tier for the 12 framework experts: a 15-run `codex exec` 0.152.1 benchmark on 2026-09-02 (3 bounded coding tasks × 5 configs, hidden tests) passed every test on every tier while Terra medium was 1.7x faster and half the cost of Sol medium, and a 6-run repetition passed 6/6. Sol keeps the judgment roles (`challenger`, `security-expert`, `prompt-engineer` at `high`, `design-expert` at `xhigh`), the analysis/research/coordination agents (`medium`) and the coordinator session (`high`); Luna `max` keeps the 5 mechanical agents. `sniper` moved to Sol/medium on 2026-09-02 (owner decision): it validates code with tooling and tests, where the benchmark showed medium equal to high; the challenger keeps high for adversarial, fresh-context review. Known risk: openai/codex#32389 (Terra intermittently returns an empty final response after tool use) is still open — the coordinator's on-disk PRD check plus the challenger and sniper gates turn it into a retry, never a silent bad merge.
...
- Assign the bare `gpt-5.6` alias, or put a judgment role (challenger, security-expert, prompt-engineer at Sol/high; design-expert at Sol/xhigh; sniper at Sol/medium) on Terra or Luna
