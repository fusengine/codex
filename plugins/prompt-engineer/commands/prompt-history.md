...
```bash
/prompts:prompt-history [action] [options]
```
...
```bash
/prompts:prompt-history show prompt_001
```
...
```bash
/prompts:prompt-history search "security"
```
...
```bash
/prompts:prompt-history diff prompt_001:v2 prompt_001:v3
```
...
```bash
/prompts:prompt-history rollback prompt_001:v2
```
...
```bash
/prompts:prompt-history export --format json
```
...
```bash
/prompts:prompt-history clean --keep 5
```
...
Every time a prompt is created or modified via `/prompts:prompt create` or `/prompts:prompt optimize`, a new version is automatically saved.

### Disable Saving

```bash
/prompts:prompt create --no-history
```

### Force Save

```bash
/prompts:prompt save --name "my-prompt" --tags "tag1,tag2"
```
