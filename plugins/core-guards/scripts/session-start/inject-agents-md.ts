#!/usr/bin/env bun
/* Bun → Python wrapper. Original preserved at _legacy_py/../_legacy_py/session-start/inject-claude-md.py. */
import { existsSync } from "node:fs";
import { homedir } from "node:os";
import { basename, dirname, join } from "node:path";

const here = dirname(import.meta.path);
const py = join(here, "../_legacy_py/session-start/inject-claude-md.py");
