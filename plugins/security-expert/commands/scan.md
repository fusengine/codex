...
- `/prompts:scan` - Full security audit of current project
- `/prompts:scan --deps-only` - Dependency audit only
- `/prompts:scan --headers` - Security headers check only
- `/prompts:scan --auth` - Authentication patterns audit only
- `/prompts:scan path/to/dir` - Scan specific directory
...
- Run `bun ${PLUGIN_ROOT}/../node_modules/@fusengine/harness/dist/cli/bin.mjs scan <dir>` for automated pattern matching
...
- `/prompts:scan` → Full OWASP + dependency + secrets audit
- `/prompts:scan --deps-only` → Quick dependency vulnerability check
- `/prompts:scan --auth` → JWT/OAuth/session security patterns
- `/prompts:scan src/api/` → Scan only API directory
