...
`spawn_agent` tool) to judge **blind**: PNG + a short brief, named elicitation lenses,
...
never overrides the owner). **Fallback**: only if `spawn_agent`/`Agent` is unavailable (agent at
