...
Via `spawn_agent`, spawn 3 parallel sub-agents, each producing exactly one lightweight direction
...
### Fallback — `spawn_agent`/`Agent` unavailable (already at nesting depth 5)
...
explored / single-fiat — spawn_agent unavailable at depth 5." Never report a "done" that implies
...
- Ran: yes/no (no → why: COMPONENT scope, no page body | spawn_agent/Agent unavailable at depth 5)
