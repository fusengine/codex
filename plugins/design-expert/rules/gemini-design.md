...
## Scope of the two lists below

The two lists that follow apply **only once the Gemini path has been chosen for a given
surface**. They are not a general obligation to use Gemini — the header above is the
governing policy, and direct HTML/CSS generation remains the default.

### FORBIDDEN once on the Gemini path
